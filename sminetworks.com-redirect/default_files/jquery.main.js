...
function initHomePagePopup() {
  if(jQuery("#home-page-popup-dialog").size() > 0 && jQuery(window).width() > 1024 && (jQuery("html").attr("lang") == "en-gb" || jQuery("html").attr("lang") == "it" || jQuery("html").attr("lang") == "pt-pt" || jQuery("html").attr("lang") == "es")) {
    jQuery(window).load(function() {
        jQuery.fancybox.open('#home-page-popup-dialog');
        setTimeout(function() {
            jQuery.fancybox.close('#home-page-popup-dialog');
        }, 10000);
    }); 
  }
}
function genFilterClasses(selectors) {
	var 	ssSel = jQuery(selectors.formSelector+" input#keyword_search"),
			searchTextBoxes = (ssSel.size() > 0) ? ssSel.val().trim() : "",
			searchClass = searchTextBoxes != "" ? ".ISSsearchMatched":"",
			filterSelected = jQuery(".filter-drop input[type='checkbox']:checked"),
			selectSelected = jQuery(selectors.formSelector+" select").filter(selectors.selSelector).filter(function() { return jQuery(this).val() != "" }),
			returnClass = ""; 
	if(filterSelected.size() > 0) {
		returnClass = filterSelected.map(function() {
		  return "."+jQuery(this).val()+searchClass
		}).get().join(",");
	} else if(selectSelected.size() > 0) {
		returnClass = selectSelected.map(function() {
		  return "."+jQuery(this).val()+searchClass
		}).get().join("")
		if(selectSelected.filter(".partner-location").size() > 0) {
			exceptLocClass = selectSelected.filter(":not(.partner-location)").map(function() {
			  return "."+jQuery(this).val()+searchClass+".all"
			}).get().join("");
			exceptLocClass = exceptLocClass == "" ? ".all" : exceptLocClass;
			returnClass += ","+exceptLocClass+searchClass;
		}
	}
	else
		returnClass = searchClass;
	return returnClass;
}
function initSelFilter(selectors) {
	jQuery(selectors.formSelector+" select").filter(selectors.selSelector).on('change', function(){
		selectors.filterSelector = genFilterClasses(selectors);
		addPagination(selectors);
	});
}
function initddFilter(selectors) {
	jQuery(".filter-drop input[type='checkbox']").off('change').on('change', function(){
		selectors.filterSelector = genFilterClasses(selectors);
		addPagination(selectors);
	});
}
function initTextfilter(selectors) {
	var searchTextBoxes = jQuery(selectors.formSelector+" input#keyword_search"),
			itemEach = jQuery(selectors.itemSelector);
  itemEach.removeClass("ISSsearchMatched");
  var sText	= searchTextBoxes.val().trim()
  if(sText != "") {
  	sText	= (!/^[a-zA-Z0-9-, ]*$/.test(sText)) ? addSlash(sText) : sText;
  	sText = sText.replace(/[,]+$/, '');
  	sText = sText.replace(/[ ,]+/g,"|");
		var pattern 	= new RegExp(sText, "i");
		itemEach.filter(function() {
			var matched = jQuery(this).find(selectors.tfSelector).filter(function() {
				return pattern.test(jQuery(this).text());
			})
			return matched.size() > 0 ? true : false;
		}).addClass("ISSsearchMatched");
  }
}
function initKWSearch(selectors) {
	jQuery(selectors.formSelector+" button[type='submit']").on('click',function(e) {
		e.preventDefault();
		initTextfilter(selectors);
		selectors.filterSelector = genFilterClasses(selectors);
		addPagination(selectors);
	});
}
function addPagination(selectors) {
	var dataSelector = selectors.itemSelector,
			paginSelector= selectors.paginSelector,
			fSelector= selectors.filterSelector || "*",
			perPage = selectors.perPage,
			datas = jQuery(dataSelector),
			fdatas = datas.filter(fSelector),
			pagination = jQuery(paginSelector);
	datas.removeClass(function (index, css) {
    return (css.match (/(^|\s)page-\S+/g) || []).join(' ');
  }).hide();
  fdatas.addClass(function(key,val) {
    return "page-"+(Math.floor(key/perPage)+1);
  }).filter(".page-1").show();
  pagination
  .pagination('destroy')
  .pagination({
    ...
    onPageClick: function(pageNumber, event) {
      datas.hide().filter(".page-"+pageNumber).show();
			jQuery('.news-container').sameHeight({
			    elements: '.item-news .text:visible',
			    multiLine: true,
			    flexible: true
			});
      jQuery('html, body').stop().animate({ scrollTop: (datas.eq(0).parent().offset().top - (jQuery("#header").outerHeight() + jQuery(".sticky-wrapper .tabset-sticky:visible").outerHeight())) - 15 +"px" }, 600);
    }
  });
}
function initPagWithFilter() {
	var selectors = [
	{
		"itemSelector" : "#block-views-block-all-resources-block-1 .flex-row .box",
		"paginSelector" : ".major-section nav.pagination",
		"formSelector" : ".search-company-form",
		"tfSelector" : ".box-holder strong.title,.box-holder h2,.body-content",
		"perPage" : 9
	},{
		"itemSelector" : ".info-section .news-container .item-news",
		"paginSelector" : ".info-section nav.pagination",
		"formSelector" : ".search-articles-form",
		"tfSelector" : "h2",
		"perPage" : 6
	},{
		"itemSelector" : ".inner-wrapper .news-container .item-news",
		"paginSelector" : ".inner-wrapper nav.pagination",
		"formSelector" : ".search-articles-form",
		"tfSelector" : "h2",
		"perPage" : 6
	},{
	  "itemSelector" : ".info-section .five-boxes .box",
	  "paginSelector" : ".info-section nav.pagination",
		"formSelector" : ".select-form",
		"selSelector" : ".success-stories",
	  "perPage" : 25
  },{
		"itemSelector" : "#eventspage .articles-list .article",
		"paginSelector" : "#eventspage nav.pagination",
		"tfSelector" : ".text-holder h2,.text-holder p",
		"selSelector" : ".month,.year",
		"formSelector" : ".filter-form",
		"perPage" : 5
  },{
		"itemSelector" : ".partner-section .tab-row-set .tab-row",
		"paginSelector" : ".partner-section nav.pagination",
		"formSelector" : ".search-company-form",
		"tfSelector" : ".description h2",
		"selSelector" : ".partner-location,.partner-type",
		"perPage" : 6
	},{
		"itemSelector" : "#block-industryandstandardsorganizations .tab-content .tab-row-set .tab-row",
		"paginSelector" : "#block-industryandstandardsorganizations nav.pagination",
		"formSelector" : ".search-articles-form",
		"tfSelector" : ".description p",
		"perPage" : 5
	}]
	for(var i=0;i<selectors.length;i++) {
		var disSelec = selectors[i];
		if(jQuery(disSelec.itemSelector).size() > 0) {
			if(typeof(disSelec.paginSelector) != "undefined") {
				var filterClass = genFilterClasses(disSelec);
				if(filterClass != "" && typeof(filterClass) != "undefined") {
					(typeof(disSelec.tfSelector) != "undefined") ? initTextfilter(disSelec) : "";
					disSelec.filterSelector = filterClass;
				}
				addPagination(disSelec);
			}
			if(typeof(disSelec.formSelector) != "undefined") {
				if(typeof(disSelec.tfSelector) != "undefined")
					initKWSearch(disSelec);
				if(typeof(disSelec.selSelector) != "undefined")
					initSelFilter(disSelec);
				initddFilter(disSelec);
			}
		}
	}
}
...
function scrollToTsection(target) {
	var aact = jQuery(".accordion-active");
	if(aact.size() > 0) {
		aact.find(".slide").slideUp(0);
		aact.removeClass("accordion-active");
	}
	jQuery(target).find("a.accordion-opener").trigger("click");
	var scrollTo = jQuery(target).offset().top,
	    doc      = jQuery('html, body'),
	    diff     = Math.abs(doc.scrollTop() - scrollTo);
	if(diff > 5)
	    doc.animate({ scrollTop: scrollTo }, 600);
}
function initSolutionSS() {
	jQuery("[data-starget]").click(function(e) {
		var tar = jQuery(this).data("starget");
		if(jQuery(tar).size() > 0) {
			e.preventDefault();
			scrollToTsection(jQuery(this).data("starget"));
		}
	});
	jQuery(window).load(function() {
		var ssTar = getParameterByName("sstarget") || "";
		if(ssTar != "") {
			scrollToTsection("#"+ssTar);
		}
	})
}
function initURLParamsSearch() {
	var ptype = { "sv" : (getParameterByName("pt") || ""), "sel" : jQuery("select.partner-type") }, 
			ploc = { "sv" : (getParameterByName("loc") || ""), "sel" : jQuery("select.partner-location") },
			cudInd = { "sv" : (getParameterByName("industry") || ""), "sel" : jQuery("select.success-stories") },
			ResCont = { "sv" : (getParameterByName("content") || ""), "sel" : jQuery("input[type='checkbox']") };
	if(ptype.sv != "") ptype.sel.val(ptype.sv).trigger("change");
	if(ploc.sv != "") ploc.sel.val(ploc.sv).trigger("change");
	if(cudInd.sv != "") cudInd.sel.val(cudInd.sv).trigger("change");
	if(ResCont.sv != "") ResCont.sel.filter("input[value='"+ResCont.sv+"']").prop("checked","checked").trigger("change");
}
//Privacy policy Internal scroll
function initppIntScroll() {
    jQuery("#pp-is li").click(function() {
        var dis = jQuery(this),
                disTarget = dis.data("target"),
                hval = jQuery("#Header.fixed").outerHeight(),
                scrollTo = parseInt(jQuery(disTarget).offset().top) - hval;
      jQuery('html,body').stop().animate({ scrollTop: scrollTo }, 500);
      dis.addClass("active").siblings().removeClass("active");
    });
}
function initCheckAll() {
	var checkboxAll = jQuery('.search-company-form input.all'),
		parent = '.filter-item';
	...
	checkboxAll.off('change').on('change', function(){
		...
		if(that.is(':checked')) {
			that.closest(parent).find('[type="checkbox"]:not(.all)').prop( 'checked', true);
		} else {
			that.closest(parent).find('[type="checkbox"]:not(.all)').prop( 'checked', false);
		}
	});
}
...
function initDropPosition() {
	var nav = jQuery('#nav'),
		secondDrop = nav.find('.sub-drop');

	secondDrop.each(function(){
		var curDrop = jQuery(this),
			parentDrop = curDrop.closest('.drop-down'),
			parentLi = curDrop.closest('li'),
			topParent = parentDrop.offset().top,
			topDrop = curDrop.offset().top;

		if (topParent > topDrop) {
			parentLi.addClass('static');
		}
	});
}
			...
			action: function(){
				if ($(window).width() >= 768) {
					if (stickyNav.sticking != 'large') {
						stickyNav.nav.sticky({ topSpacing: 0, zIndex: 9, center:true, className:"hey", getWidthFrom: '#main', responsiveWidth: true });
...
function initDropDownClasses() {
	...
	lib.each(lib.queryElementsBySelector('.sub-drop ul li'), function(index, item) {
...
function initAccordion() {
	...
	jQuery('.drop-box').slideAccordion({
		addClassBeforeAnimation: true,
		opener: '.accordion-opener-inner',
		slider: '.drop-inner',
		cloneLink: true,
		animSpeed: 300
	});
	jQuery('.accordion').not("#subnav").slideAccordion({
		addClassBeforeAnimation: true,
		opener: 'a.accordion-opener',
		slider: '.drop',
		animSpeed: 300
	...
	//p2
	jQuery('.accordion#subnav').slideAccordion({
		addClassBeforeAnimation: true,
		opener: 'a.accordion-opener',
		slider: '.drop',
		cloneLink: true,
	...
	jQuery('.filters-row').slideAccordion({
		addClassBeforeAnimation: true,
		opener: 'a.filter',
		slider: '.filter-drop',
		animSpeed: 300
	...
	function innerAccordion() {
		var container = jQuery('.sub-drop'),
			animSpeed = 300,
			activeClass = 'accordion-active';

		container.each(function(){
			var that =  jQuery(this),
				slide = that.find('ul ul');

			slide.slideUp(0);

			slide.each(function(){
				var curSlide = jQuery(this),
					parent = curSlide.closest('li'),
					openerLink = parent.find('>a');


				jQuery('<li class="visible-tablet clone"></li>').prependTo(curSlide);
				openerLink.clone().removeAttr('class').prependTo(curSlide.find('.clone').first()).text('Overview');
				

				openerLink.on('click', function(e){
					var actParent = container.find('ul> .accordion-active');
					if(jQuery('#header .nav-opener').is(':hidden')) {
						return;
					} else {
						e.preventDefault();
					}
	...
	/*Auto Hide - on click outside - P2*/
	jQuery(document).on("click",function(e) {
		var dis = jQuery(e.target),
				activedd = jQuery(".filter-item.accordion-active");
		if(dis.parents(".filter-item").size() <= 0 && !dis.hasClass(".filter-item") && activedd.size() > 0) {
			activedd.find("a.filter").trigger("click");
		}
	});
	/* filter close - on click close btn p2 */
	jQuery(".search-company-form .filter-drop .filter-close").on("click",function(e) {
		jQuery(".filter-item.accordion-active").find("a.filter").trigger("click");
	});
	innerAccordion();
}
 	...
 	$.fn.slideAccordion = function(opt){
		...
		return this.each(function(){
			...
			items.each(function(){
				...
				if(options.cloneLink) {
					var linkContainer = slider.find('ul').first();
					//clone opener link to accordion
					jQuery('<li class="visible-tablet clone"></li>').prependTo(linkContainer);
					opener.clone().removeAttr('class').prependTo(slider.find('ul > .clone')).text('Overview');
				}
				...
				opener.bind(options.event, function(e){
					...
					if((jQuery(e.target).closest('#nav').length || jQuery(e.target).closest('#subnav').length) && jQuery('#header .nav-opener').is(':hidden')) {
...
/** simplePagination.js v1.6  http://flaviusmatis.github.com/simplePagination.js/ */
