...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Initial animation for slide-down elements on page load
  ...
  // CONTACTO FLOATING FORM LOGIC - New and improved version
  function initContactForms() {
    console.log("Initializing contact form functionality");
    
    // Get all contact form elements - using document.querySelector to ensure we look at the entire DOM
    const contactoBtn = document.querySelector('#btn-contacto');
    const contactoBtnDesktop = document.querySelector('#btn-contacto-desktop');
    const contactoFloat = document.querySelector('#contacto-float');
    const closeContacto = document.querySelector('#close-contacto-float');
    const contactoForm = document.querySelector('#contacto-form');
    const contactoSuccess = contactoForm ? contactoForm.querySelector('#contacto-success') : null;
    const contactoError = contactoForm ? contactoForm.querySelector('#contacto-error') : null;
    ...
    const btn1 = document.querySelector('#btn-solicita1');
    const btn2 = document.querySelector('#btn-solicita2');
    ...
    if (contactoFloat && closeContacto && contactoForm) {
      ...
      contactoForm.addEventListener('submit', function(e) {
        ...
        // Send to Google Sheet - make sure GOOGLE_SHEET_URL is defined in settings.js
        try {
          ...
          .catch(error => {
            ...
            // Fall back to showing success anyway since we can't reliably detect errors
            if (successDiv) successDiv.style.display = 'block';
            if (errorDiv) errorDiv.style.display = 'none';
            setTimeout(() => {
              if (successDiv) successDiv.style.display = 'none';
          ...
          // Show success anyway as a fallback
          if (successDiv) successDiv.style.display = 'block';
          if (errorDiv) errorDiv.style.display = 'none';
          setTimeout(() => {
            if (successDiv) successDiv.style.display = 'none';
  ...
  // Call the initialization function at the end of the page load to ensure all elements are available
